...
public abstract class VmAllocationPolicy {
	...
	public boolean allocateHostForGuest(GuestEntity guest, HostEntity host) {
		...
		if (host == guest) { // cannot be hosted on itself (VirtualEntity edge-case)
			Log.printlnConcat(CloudSim.clock(), ": ", datacenterName, ".vmAllocator: Allocation of ", 
			guest.getClassName(), " #", guest.getId(), " to ", host.getClassName(), " #", host.getId(), " failed (cannot be allocated on itself)");
			return false;
		}
